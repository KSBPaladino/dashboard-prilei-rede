...
# 2. CONSTRUÇÃO DO DASHBOARD (Acesso Livre)
...
try:
    ...
    # FILTRO 1: Edital
    editais_disponiveis = sorted(df['Edital'].astype(str).unique())
    edital_selecionado = st.sidebar.multiselect("Edital:", options=editais_disponiveis, default=editais_disponiveis)
    df_temp = df[df['Edital'].astype(str).isin(edital_selecionado)]

    # FILTRO 2: Instituição
    inst_disponiveis = sorted(df_temp['Instituição da Rede'].unique())
    inst_selecionada = st.sidebar.multiselect("Instituição:", options=inst_disponiveis, default=inst_disponiveis)
    df_temp = df_temp[df_temp['Instituição da Rede'].isin(inst_selecionada)]
    ...
    if df_filtrado.empty:
        ...
        # --- BLOCO ADICIONADO: Exibição de Editais e Instituições ---
        texto_edital = ', '.join(edital_selecionado) if len(edital_selecionado) < len(editais_disponiveis) else 'Todos'
        texto_inst = ', '.join(inst_selecionada) if len(inst_selecionada) < len(inst_disponiveis) else 'Todas'

        st.info(f"**Editais:** {texto_edital} ｜ **Instituições:** {texto_inst}")
        # -------------------------------------------------------------
        ...
        with col5:
            ...
            fig5 = px.treemap(df_filtrado, path=['Edital', 'Instituição da Rede', 'Curso', 'Município'],
